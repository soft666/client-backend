...
<script>
import {apiViewEventDetail, apiImg, apiViewEventName} from '../service/backend'
...
export default {
  ...
  methods: {
    album () {
      this.$http.get(apiViewEventDetail + this.$route.params.id).then((response) => {
    ...
    albumname () {
      this.$http.get(apiViewEventName + this.$route.params.id).then((response) => {
